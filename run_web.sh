#!/bin/bash
# start_server.sh - Запуск сервера DuoNet (аналог run_web.sh)
...
echo -e "\n${YELLOW}[1/7] Проверка SSL сертификата...${NC}"
mkdir -p "$SSL_DIR"
...
echo -e "\n${YELLOW}[2/7] Проверка баз данных...${NC}"
...
echo -e "\n${YELLOW}[4/7] Проверка Rendezvous...${NC}"
...
echo -e "\n${YELLOW}[5/7] Запуск API сервера (HTTPS)...${NC}"
...
# Регистрация NAT-сервера в Rendezvous (если есть серверный аккаунт)
echo -e "\n${YELLOW}[7/7] Регистрация NAT-сервера в Rendezvous...${NC}"
python3 -c "
import sqlite3
...
echo ""
echo -e "${GREEN}=========================================="
echo "✅ DuoNet сервер запущен!"
echo -e "==========================================${NC}"
echo ""
echo -e "${BLUE}🔗 Доступ:${NC}"
echo "  • Веб-интерфейс: https://localhost:$API_PORT"
echo "  • Swagger API: https://localhost:$API_PORT/docs"
echo "  • Rendezvous API: http://localhost:$RENDEZVOUS_PORT/api/health"
echo ""
echo -e "${BLUE}📁 Файлы:${NC}"
echo "  • Пользовательская БД: duonet.db"
echo "  • Серверная БД: duonet_server.db"
echo "  • Логи: logs/api.log, logs/rendezvous.log"
echo ""
echo -e "${YELLOW}⚠️  Внимание:${NC}"
echo "  • Используется самоподписной SSL сертификат"
echo "  • Примите предупреждение браузера"
echo ""
echo -e "${YELLOW}🛑 Для остановки: Ctrl+C${NC}"

# Обработка остановки
trap 'echo -e "\n${YELLOW}Остановка серверов...${NC}"; stop_processes; echo -e "${GREEN}✓ Серверы остановлены${NC}"; exit' INT TERM
